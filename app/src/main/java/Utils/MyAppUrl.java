package Utils;

/**
 * Created by Arsalan khan on 10/18/2017.
 */

public class MyAppUrl {

    public static  String HOME_REMEDIES_URL="http://projects.multibizservices.com/HEALTHY_HOME/" +
                                                   "healthy_home/sub_categories.php?category_id=1";


    public static String HOME_REM_SUB_CATEGORY_URL= "http://projects.multibizservices.com/" +
                                                           "HEALTHY_HOME/healthy_home/category_details.php?sub_category_id=";


    public static String HEALTH_BENEFITS_URL= "http://projects.multibizservices.com/HEALTHY_HOME/healthy_home/sub_categories.php?category_id=4";

    public static String HEALTH_BENEFITS_CATEGORY_DETAIL_URL="http://projects.multibizservices.com/HEALTHY_HOME/healthy_home/health_detail.php?sub_id=";
}
